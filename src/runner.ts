...
export class TestRunner {
  ...
  private async executeTest(
    ...
    // Get the test code from either 'code' or 'testCode' field
    const testCode = test.code || test.testCode;

    // Check if we should use AI-generated or recorded tests
    if ((this.config.testGenerationMode === 'ai' || this.config.testGenerationMode === 'record') && testCode) {
      // Let AI/recorded test run completely - errors will be handled by caller
      return await this.executeAIGeneratedTest(page, test, baseUrl, screenshotPath);
    }

    // Use simple auto-test for template mode or when test code is unavailable
  ...
  private async executeAIGeneratedTest(
    ...
    try {
      // Get the test code from either field
      const testCode = test.code || test.testCode;
      if (!testCode) {
        throw new Error('No test code available');
      }

      // Transpile TypeScript to JavaScript
      const jsCode = typescript.transpileModule(testCode, {
        compilerOptions: {
